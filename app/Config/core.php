...
if ( file_exists( APP . 'Config/config.php' ) )
{
	require_once( APP . 'Config/config.php' );
        require_once(APP.'Config/general.php');
        $debug_mode = $config['system']['production_mode'];
    if(isset($_SERVER['REQUEST_URI']) && strpos($_SERVER['REQUEST_URI'], '/upgrade') !== false){
        $debug_mode = 2;
    }
 ...
 */
	Configure::write('Security.salt', $CONFIG['salt']);

/**
 * A random numeric string (digits only) used to encrypt/decrypt strings.
 */
	Configure::write('Security.cipherSeed', $CONFIG['cipher']);
...
Cache::config('_cache_group_', array(
    'engine' => $engine,
    'duration' => $duration,
    'groups' => array('cache_group')
));
