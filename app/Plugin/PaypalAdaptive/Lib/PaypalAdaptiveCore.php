...
class PaypalAdaptiveCore extends PaymentGatewayCore
{
		...
		if($ipnMessage->validate()) 
		{
			...
			else
			{
				...
				else
				{
					...
					$helperPlugin->onSuccessful($item,$data,$data['transaction']['receiver']['amount']);
				}
			}
	...
	public function ipnPreapprovalItem()
	{
		...
		if($ipnMessage->validate()) 
		{
			...
			if ($data['status'] == 'ACTIVE')
			{
				...
				$helperPlugin->onSuccessful($item,$data,$params['amount']);
			}
	...
	public function payWithPreapproval($type,$id,$params_pre)
	{
		...
		$helperPlugin->onSuccessful($item,$params_pre,$params['amount'],true);
