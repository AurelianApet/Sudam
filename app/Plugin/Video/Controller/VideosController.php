...
class VideosController extends VideoAppController
{
	...
	public function index($cat_id = null)
	{
        ...
        public function profile_user_video($uid = null) {
            $uid = intval($uid);
            ...
            $videos = $this->Video->getVideos('user', $uid, $page,RESULTS_LIMIT,'',0,$are_friend,$role_id);
            $more_videos = $this->Video->getVideos('user', $uid, $page + 1,RESULTS_LIMIT,'',0,$are_friend,$role_id);
            $this->set('videos', $videos);
	...
	********************/
	...
	public function view( $id = null )
	{
		...
		$this->set('title_for_layout', htmlspecialchars($video['Video']['title']));
	...
	private function _getVideoDetail( $video )
	{
            ...
            $cakeEvent = new CakeEvent('Plugin.Controller.Video.getVideoDetail', $this, array('uid' => $uid, 'video' => $video, 'admins' => $admins));
            $this->getEventManager()->dispatch($cakeEvent);
