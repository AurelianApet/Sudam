...
class SubscriptionPackagesController extends SubscriptionAppController
{
    ...
    public function admin_create($id = null)
    {
        ...
        else if((int)$id > 0 && !$this->SubscriptionPackage->hasAny(array('id' => (int)$id)))
        {
        ...
        else 
        {
            if((int)$id > 0)
            {
                ...
                $this->set('pricingPlan', $package['SubscriptionPackagePlan']);
            }
            else
            {
                $disable = false;
    ...
    {
        if(!$this->SubscriptionPackage->hasAny(array('id' => (int)$id)))
        {
            $this->Session->setFlash(__( 'This package does not exist'), 'default', array('class' => 'Metronic-alerts alert alert-danger fade in' ));
        }
        else 
