...
class MooTextHelper extends TextHelper{
    ...
    function convert_clickable_links_for_hashtags($message,$enable = true)
    {
        ...
        preg_match_all('/(^|\s)#(\w*[a-zA-Z_]+\w*)/', strip_tags($message), $matchedHashtags);
        $hashtag = '';
    ...
    public function mentionLink($match){
        ...
        $link =$this->Html->link($match[2],array('plugin' => false, 'controller' => 'users','action' => 'view',$match[1]),array("class" => $class, "data-item_type" => "user", "data-item_id" => $match[1]));
        return $link;
    }
