    ...
    */
    ...
    protected function _logMeIn($email, $password, $remember = false)
    {
        ...
        if (!empty($user)) // found
        {
            ...
            $auto_disable_reach_max_age = Configure::read('core.auto_disable_reach_max_age');
    ...
    public function doLoadingBlocks($uri)
    {
        if ($this->layout != '' && $this->autoRender != false && !$this->request->is('post') && !$this->request->is('requested')) {
            ...
            $blocks = Cache::read("$uri.blocks".Configure::read('Config.language'));
            if (!$blocks) {
                ...
                Cache::write("$uri.blocks".Configure::read('Config.language'), $row);
                $blocks = $row;
            }
    ...
    private function doThemeProcess()
    {
		...
		if (isset($this->request->data['language']) && $this->request->data['language'] && is_string($this->request->data['language']))
		{
