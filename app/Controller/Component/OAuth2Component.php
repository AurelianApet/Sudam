...
class OAuth2Component extends Component {
    ...
    public function verifyResourceRequest($excludeActions = array()) {
        ...
        if (is_null($token)) {
        	if ($this->_Collection->getController()->isApp())
        	{
        		$this->_Collection->getController()->theme = 'mooApp';
        		echo $this->_Collection->getController()->render('/Elements/token_error');
        		die();
        	}
    ...
    private function validateResourceOwnerPasswordCredentials() {
        ...
        if (empty($data['username'])) {
            throw new BadRequestException(__d('api','Missing parameter : username is REQUIRED'));
        }

        if (empty($data['password'])) {
            throw new BadRequestException(__d('api','Missing parameter : password is REQUIRED'));
        }
      
        // Validate email
        if (filter_var($data['username'], FILTER_VALIDATE_EMAIL) === false) {
            throw new BadRequestException(__d('api','Parameter error : please enter a valid email'));
        }
